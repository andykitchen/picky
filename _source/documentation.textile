...
in the "repository":http://github.com/floere/picky.
...
So all the indexes can be viewed in the @/index@ directory of the project. They are saved as JSON and as text for the preprocessing step.
So if anything does not work, you can go see how it is indexed and change your indexing parameters accordingly.

Also, since all is Ruby, you can log as much data as you want to help you improve your search application until it's working perfectly.
...
When the server is started, they are loaded into memory. As soon as the server is stopped, the indexes are not in memory again. In the Sinatra server, you have to load them your
...
Indexing regenerates the indexes in the Redis server.
...
Also note that these options need to make it through the tokenizing ("searching":#searching).
...
By default, an index is identified by its *name* in the results:
...
This index is identified by @:books@.
...
This index is identified by @:media@ but you still refer to it as @:books@ in e.g. Rake tasks, @Picky::Indexes[:books].reload@.
...
h2(#indexing). Indexing / Tokenizing

Indexing describes how *text data from the data source* is handled.
...
h2(#searching). Searching / Tokenizing

Indexing describes how *queries* are handled.
...
# *removes_characters_after_splitting*: Regexp on which characters to remove after the splitting.
# *normalizes_words*: [[/matching_regexp/, 'replace match \1']]
# --*rejects_token_if*: lambda { |gets_token| gets_token == :hello }-- (not yet)
# *case_sensitive*: true or false, false is default.
# *substitutes_characters_with*: A character substituter that responds to @#substitute(text) #=> substituted text@
