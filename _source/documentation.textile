...
### "Early Termination":#search-options-earlytermination
# "Results":#results
...
h4(#search-options-earlytermination). Early Termination
<div class="tags">performance</div>

The @early_termination(integer)@ or @early_termination(with_extra_allocations: integer)@ option stops Picky from calculate all ids of all allocations.
...
<pre><code>
phone_search = Search.new phonebook do
  early_termination # The default uses 0.
end
</code></pre>
...
<pre><code>
phone_search = Search.new phonebook do
  early_termination with_extra_allocations: 5
end
</code></pre>
