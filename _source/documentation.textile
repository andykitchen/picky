...
### "Searching":#search-options-searching
### "Boost":#search-options-boost
### "Ignore Unassigned Tokens":#search-options-ignore
...
h4(#search-options-searching). Searching

See "below":#searching for detailed @searching@ options.
...
h4(#search-options-ignore). Ignore Unassigned Tokens
...
In short the @ignore_unassigned_tokens@ option makes Picky be very lenient with your queries. Usually, if one of the search words is not found, say in a query "aston martin cockadoodledoo", Picky will return an empty result set, because "cockadoodledoo" is not in any index, in a car search, for example.
